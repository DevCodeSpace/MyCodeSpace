...
class _LineChartSampleState extends State<LineChartSample> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        AspectRatio(aspectRatio: 2, child: LineChart(mainData())),
      ],
    );
  }

  Widget bottomTitleWidgets(double value, TitleMeta meta) {
    final style = regularTextStyle(
      13,
  ...
  Widget leftTitleWidgets(double value, TitleMeta meta) {
    final style = regularTextStyle(13, color: widget.textColor);
    String text = '';
    ...
    return LineChartData(
      gridData: FlGridData(
        ...
        getDrawingHorizontalLine: (value) {
          return FlLine(
            color: widget.horizontalLinesColor.withValues(alpha: .3),
            strokeWidth: 1,
          );
        },
        getDrawingVerticalLine: (value) {
          return FlLine(
            color: widget.verticalLinesColor.withValues(alpha: .3),
            strokeWidth: 1,
          );
        },
      ),
      ...
      borderData: FlBorderData(
        ...
        border: Border.fromBorderSide(
          BorderSide(color: widget.textColor.withValues(alpha: .3)),
        ),
      ),
      minX: 0,
      maxX: 4,
