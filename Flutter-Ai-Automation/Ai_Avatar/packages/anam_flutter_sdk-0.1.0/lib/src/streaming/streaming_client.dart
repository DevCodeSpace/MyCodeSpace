...
class StreamingClient {
  ...
  Future<void> initializePeerConnection() async {
    ...
    try {
      ...
      _peerConnection!.onConnectionState = (RTCPeerConnectionState state) {
        ...
        } else if (state == RTCPeerConnectionState.RTCPeerConnectionStateFailed || state == RTCPeerConnectionState.RTCPeerConnectionStateClosed) {
          _isConnected = false;
  ...
  Future<Map<String, dynamic>> createOffer() async {
    try {
      ...
      _logger.d('📋 Created offer (trickle ICE — candidates will be sent separately)');
  ...
  Future<void> addIceCandidate(dynamic rawCandidate) async {
    try {
      ...
      if (rawCandidate is Map) {
        ...
        _logger.w('Skipping ICE candidate — unexpected type: ${rawCandidate.runtimeType}');
        return;
      }

      // sdpMLineIndex may arrive as a String from some servers.
      final mLineIndex = candidate['sdpMLineIndex'] is int ? candidate['sdpMLineIndex'] as int : int.tryParse(candidate['sdpMLineIndex']?.toString() ?? '') ?? 0;
