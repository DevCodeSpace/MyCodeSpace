...
class SignalingClient {
  ...
  Future<void> connect() async {
    try {
      ...
      if (proxyUrl != null) {
        ...
        final hostUri = Uri.parse(engineHost.contains('://') ? engineHost : 'https://$engineHost');
        final baseHost = hostUri.host;
        final basePort = hostUri.port;

        final endpointPath = signallingEndpoint.startsWith('/') ? signallingEndpoint : '/$signallingEndpoint';
  ...
  void _handleMessage(dynamic data) {
    try {
      ...
      if (messageType == 'heartbeat') {
        ...
        final reason = (rawPayload is Map ? rawPayload['reason'] : null) ?? message['reason'] ?? 'No reason provided';
        _logger.w('⚠️ Session ended by server: ${message['sessionId']}. Reason: $reason');
        ...
        final payload = message['payload'];
        if (payload != null && payload is Map && payload['connectionDescription'] != null) {
          _messageController.add({
            'type': 'answer',
            'data': Map<String, dynamic>.from(payload['connectionDescription'] as Map),
          });
        } else {
  ...
  void _handleDisconnect() {
    ...
    if (_reconnectAttempts < _maxReconnectAttempts) {
      ...
      _logger.i('Attempting to reconnect in ${delay.inMilliseconds}ms (Attempt $_reconnectAttempts/$_maxReconnectAttempts)');
      Timer(delay, () => connect());
    } else {
  ...
  void sendIceCandidate(dynamic candidate) {
    final payload = candidate is RTCIceCandidate ? candidate.toMap() : candidate;
    sendMessage({
      'type': 'icecandidate',
