...
class AnamClient {
  ...
  Future<void> _ensureSessionToken() async {
    if (options.sessionToken == null && options.apiKey != null) {
      try {
        final sessionToken = await _apiClient.getSessionToken(apiKey: options.apiKey!);
    ...
    try {
      ...
      if (preNegotiatedSession != null) {
        ...
        _streamingClient = StreamingClient(
          ...
          iceServers: (clientConfig['iceServers'] as List<dynamic>?)?.map((e) => e as Map<String, dynamic>).toList(),
        );
      ...
      try {
        await _signalingClient!.messages
            .firstWhere((msg) => (msg['type'] ?? msg['actionType']) == 'registered')
            .timeout(const Duration(milliseconds: 1500));
        _logger.d('✅ Registration confirmed by server');
      } catch (e) {
        _logger.w('⚠️ Registration acknowledgment timed out or failed — proceeding anyway');
      }
  ...
  void _setupSignalingHandlers() {
    _signalingClient!.messages.listen((message) async {
      try {
        ...
        switch (messageType) {
          ...
          case 'signaling_closed':
            ...
            _logger.d('📡 Signaling WebSocket closed (expected after ICE exchange)');
            break;
          default:
      ...
      if (error is ClientError && error.message.contains('Session ended by server')) {
  ...
  void _setupStreamingHandlers(Function(MediaStream?) onStreamReady) {
    _streamingClient!.eventEmitter.on<dynamic>(AnamEvent.videoStreamStarted).listen((stream) {
      _logger.d('🎬 Video stream started event received');
      onStreamReady(stream as MediaStream?);
    });

    _streamingClient!.eventEmitter.on<dynamic>(AnamEvent.iceConnectionStateChanged).listen((data) {
      if (data != null && data is Map && data['candidate'] != null) {
        _signalingClient!.sendIceCandidate(data['candidate']);
      }
    });

    _streamingClient!.eventEmitter.on<dynamic>(AnamEvent.dataChannelMessage).listen((data) {
...
class AnamClientFactory {
  ...
  static Future<Map<String, dynamic>> createWebRTCOffer({
    ...
    try {
      ...
      final sub = tempEventEmitter.on<dynamic>(AnamEvent.iceConnectionStateChanged).listen((data) {
