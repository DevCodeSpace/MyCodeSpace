...
class UsageStats {
  ...
  static Future<List<EventUsageInfo>> queryEvents(DateTime startDate, DateTime endDate) async {
    ...
    List<EventUsageInfo> result = events.map((item) => EventUsageInfo.fromMap(item)).toList();
    return result;
  }

  static Future<List<ConfigurationInfo>> queryConfiguration(DateTime startDate, DateTime endDate) async {
    int end = endDate.millisecondsSinceEpoch;
    int start = startDate.millisecondsSinceEpoch;
    Map<String, int> interval = {'start': start, 'end': end};
    List configs = await _channel.invokeMethod('queryConfiguration', interval);
    List<ConfigurationInfo> result = configs.map((item) => ConfigurationInfo.fromMap(item)).toList();
    return result;
  }

  static Future<List<EventInfo>> queryEventStats(DateTime startDate, DateTime endDate) async {
    int end = endDate.millisecondsSinceEpoch;
    int start = startDate.millisecondsSinceEpoch;
    Map<String, int> interval = {'start': start, 'end': end};
    List eventsStats = await _channel.invokeMethod('queryEventStats', interval);

    List<EventInfo> result = eventsStats.map((item) => EventInfo.fromMap(item)).toList();
    return result;
  }
  ...
  static Future<List<UsageInfo>> queryUsageStats(DateTime startDate, DateTime endDate) async {
    ...
    List<UsageInfo> result = usageStats.map((item) => UsageInfo.fromMap(item)).toList();
    return result;
  }

  static Future<Map<String, UsageInfo>> queryAndAggregateUsageStats(DateTime startDate, DateTime endDate) async {
    int end = endDate.millisecondsSinceEpoch;
    int start = startDate.millisecondsSinceEpoch;
    Map<String, int> interval = {'start': start, 'end': end};
    Map usageAggStats = await _channel.invokeMethod('queryAndAggregateUsageStats', interval);
    Map<String, UsageInfo> result = usageAggStats.map((key, value) => MapEntry(key as String, UsageInfo.fromMap(value)));
    return result;
  }
  ...
  static Future<List<NetworkInfo>> queryNetworkUsageStats(
    ...
    List<NetworkInfo> result = events.map((item) => NetworkInfo.fromMap(item)).toList();
    return result;
  }
  ...
  static Future<NetworkInfo> queryNetworkUsageStatsByPackage(
    ...
    Map response = await _channel.invokeMethod('queryNetworkUsageStatsByPackage', interval);
